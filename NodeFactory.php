...
class NodeFactory {
  private $entity = null;
  private $entity_wrapper = null;
  private $actioned = "created";

  public __init__($node_type=null) {
    $explicit_values = $this->set_verifiable_options();
    if (!$node_type) {
      $interactive = true;
      $implicit_values = create_node_interactively();
      $body = $this->get_body_interactively();
    } else {
      $interactive = false;
      $node_values['type'] = $node_type;
      $implicit_values = get_simple_node_options();
      $body = $this->get_body();
    }

    $entity = $this->get_entity(array_merge($explicit_values, $implicit_values));

    if ($body) {
      $entity->body->set($body);
    }

    return $entity;
  }

  public get_entity($values=null) {
    if (!$this->entity) {
  ...
  public create_node_interactively() {
  ...
  public set_field_reference($field_name, $value) {
    $value = make_ints($value);
    $this->get_entity()->$field_name->set($value);
  }

  public get_body_interactively() {
  ...
  public create_complex_fields_interactively() {
  ...
  public get_simple_node_options() {
    $node_values = array();
  ...
  public get_body() {
    ...
    if ($body['value']) {
      ...
      $body['safe_value'] = strip_tags($body, "<p>");
      $body['safe_summary'] = strip_tags($summary, "<p>");
      return $body;
    }
    return null;
  }

  public set_verifiable_options() {
    $node_values = array();
